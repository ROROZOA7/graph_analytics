...
class Corpus(object):
    def __init__(self, corpus_folder=None, extn='WL2', max_files=0):
        ...
        self.subgraph_index = 0 # Number of subgraph
        self.graph_index = 0
        self.epoch_flag = 0
    ...
    def generate_batch_pvdm(self, batch_size):
        ...
        while len(context_subgraph_ids) < batch_size:
            line_id = self.subgraph_index
            context_subgraph = graph_contents[line_id].split()[0]
            context_output = graph_contents[line_id].split()[-1]
            # context_subgraph = graph_contents[line_id].split()
            target_graph = graph_name

            context_subgraph_ids.append(self._subgraph_to_id_map[context_subgraph])
            target_graph_ids.append(self._graph_name_to_id_map[target_graph])
            context_output_ids.append(self._subgraph_to_id_map[context_output])
        ...
        target_context_pairs = zip(target_graph_ids, context_subgraph_ids, context_output_ids)
        shuffle(target_context_pairs)
        target_graph_ids, context_subgraph_ids, context_output_ids = zip(*target_context_pairs)

        target_graph_ids = np.array(target_graph_ids, dtype=np.int32)
        context_subgraph_ids = np.array(context_subgraph_ids, dtype=np.int32)

        contextword_outputs = np.reshape(context_output_ids, [len(context_output_ids), 1])
        return target_graph_ids,context_subgraph_ids , contextword_outputs
